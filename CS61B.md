...
| **关键字**     | **本类内部 (Class)** | **同一个包 (Package)** | **子类 (不同包) (Subclass)** | **全局 (World)** | **适用场景简述**             |
| ----------- | ---------------- | ------------------ | ----------------------- | -------------- | ---------------------- |
| `private`   | ✅                | ❌                  | ❌                       | ❌              | 仅限类内部使用（如：敏感数据、内部辅助方法） |
| (无)         | ✅                | ✅                  | ❌                       | ❌              | 包内协作（如：单元测试、包内工具类）     |
| `protected` | ✅                | ✅                  | ✅                       | ❌              | 允许子类继承扩展（即使子类在不同包）     |
| `public`    | ✅                | ✅                  | ✅                       | ✅              | 对外公开的 API、接口、Main 方法   |
## static
...
| **修饰目标** | **代码示例**                      | **含义**                                 |
| -------- | ----------------------------- | -------------------------------------- |
| **变量**   | `private final int age = 18;` | **“绝育”**：赋值后，值（或引用）**不可更改**。           |
| **类**    | `public final class String`   | **“绝后”**：这个类**不能被继承** (不能 extends)。    |
| **方法**   | `public final void eat() {}`  | **“一言九鼎”**：子类**不能重写** (Override) 这个方法。 |
# 3、Testing
## Junit
...
## 随机插入BST
1. 对于随机插入的BST，平均高度约为**2ln(N)**。
2. 每次put和containsKey操作平均需要 **ln(N)** 次比较。
...
## java泛型数组
```java
// 假设 buckets 声明为 Collection<Node>[] buckets;
// 下面这行会直接红线报错：Generic array creation
buckets = new Collection<Node>[10];

// 创建数组时使用“原生类型”（去掉 <Node>）
buckets = new Collection[10];
```
## 遍历时remove
- **如果在遍历集合时想要删除元素，使用 `Iterator.remove()` 是唯一安全的方法**
- 删除的是前一次next()所对应的原集合的元素（类似`电脑光标`）
- 如果还没有调用第一次next()就调用remove()会报错
- 调用第一次next()后连续调用两次remove()也会报错
```java
// 哈希表中remove()方法的一部分
